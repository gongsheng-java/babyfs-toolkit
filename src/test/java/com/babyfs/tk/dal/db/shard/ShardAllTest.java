...
public class ShardAllTest {
    ...
    private void test(List<? extends EntityShard.IShardStrategy> dbShards, List<? extends EntityShard.IShardStrategy> tableShards) {
        //setup db instance
        DBInstance dbInstance_remote = new DBInstance("db_0", "127.0.0.1", 3306, "root", "");
        ShardDataSourceContainer shardDataSourceContainer = new ShardDataSourceContainer(new TomcatDataSourceCreator());
        //ShardDataSourceContainer shardDataSourceContainer = new ShardDataSourceContainer(new C3p0DataSourceCreator());
    ...
    public static void testShardFriend(IShardFriendDao iShardFriendDao) {
        ...
        for (int i = 1; i <= count; i++) {
            ...
            Assert.assertNotNull(shardFriend);
            Assert.assertEquals((i), shardFriend[0]);
