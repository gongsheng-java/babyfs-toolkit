...
public class DefaultMetricNamer implements MetricNamer {
    ...
    static final String CACHED_GAUGE_SUFFIX = "gauge";
    ...
    @Nonnull
    @Override
    public String getNameForGauge(@Nonnull Method method, @Nonnull CachedGauge gauge) {
        if (gauge.absolute()) {
            return gauge.name();
        }

        if (gauge.name().isEmpty()) {
            return name(method.getDeclaringClass(), method.getName(), CACHED_GAUGE_SUFFIX);
        }
