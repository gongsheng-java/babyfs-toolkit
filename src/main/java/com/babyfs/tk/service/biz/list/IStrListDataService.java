...
/**
 * {@link BaseStrListEntity}列表数据服务,使用string作为ownerId
 *
 * @param <L> 列表实体的类型
 * @param <E> 列表计数实体的类型
 */
public interface IStrListDataService<L extends BaseStrListEntity, E extends BaseStrListCounterEntity> {
    /**
     * 增加列表一个实体
     *
     * @param entity entity
     ...
     * @return 操作是否成功
     */
    boolean del(String ownerId, long targetId);
     ...
     * @return
     */
    Long getOwnerTargetId(String ownerId, long targetId);
     ...
     * @return target id
     */
    Pair<Long, List<Long>> loadList(String ownerId, int page, int pageSize, long cursor);
     ...
     */
    Pair<Long, List<Pair<Long, Long>>> loadListWithScoreId(String ownerId, int page, int pageSize, long cursor);
}
