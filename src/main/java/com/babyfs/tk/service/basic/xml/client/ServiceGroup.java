...
public class ServiceGroup {
    ...
    public static final class ClientMapAdapter extends XmlAdapter<ClientConfigType, Groups> {
        ...
        public Groups unmarshal(ClientConfigType v) throws Exception {
            Map<String, Group> tGroups = Maps.newHashMap();
            for (Group p : v.getGroups()) {
                Preconditions.checkArgument(!tGroups.containsKey(p.getName()), "Duplicate key %s", p.getName());
                tGroups.put(p.getName(), p);
            }
            return new Groups(tGroups, v.getProperties());
        }
